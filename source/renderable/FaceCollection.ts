class FaceCollection extends Array<Face> {
    //Faces: Array<Face> = [];
    constructor(...faces: Face[]) {
        super(...faces);
        (<any>Object).setPrototypeOf(this, FaceCollection.prototype);
    }
    Render(param: RenderParam, _context: IContext): string {
        var retVal: string = "";
        if (this.length == 0) {
    ...
    static Format(format: string, object: any): string {
        var items = Object.getOwnPropertyNames(object).map<{ value: any, pattern: string }>((x, i) => { return { value: object[x]||'', pattern: `@col${i + 1}` }; }).reverse();
